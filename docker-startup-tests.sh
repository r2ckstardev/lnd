...
ready() { docker logs "$LND" 2>&1 | grep -Eq 'Wallet unlocked|Migrated wallet|Wallet initialized'; }
reset_complete() { docker exec "$LND" test ! -e /data/.macaroon-reset-pending; }
...
for SCENARIO in ${TEST_SCENARIOS:-legacy newline stored-newline empty null omitted password-only custom custom-newline custom-stored-newline custom-long pending-before pending-after rotation rotation-newline rotation-custom rotation-custom-newline rotation-pending rotation-split-store interrupted-reset missing-store-old missing-store-new missing-readonly split-store unknown invalid-json fresh interrupted-init}; do
    ...
    case "$SCENARIO" in
        ...
        custom-long) ACTUAL=$'  long password with spaces * ? [brackets] and more than fifty-seven bytes\r\n\n'; STORED=$ACTUAL ;;
    ...
    if [[ "$SCENARIO" != fresh ]]; then
        ...
        case "$SCENARIO" in
            interrupted-reset) offline 'touch /data/.macaroon-reset-pending' ;;
        ...
        case "$SCENARIO" in
            ...
            *split-store) offline 'mv /data/old-store /data/data/chain/bitcoin/regtest/macaroons.db' ;;
            ...
            interrupted-init)
                ...
                # new volume with the saved initialization request.
    ...
    case "$SCENARIO" in
        ...
        *)
            ...
            if docker exec "$LND" test -e "$WALLET.newpassword"; then
                [[ $(docker exec "$LND" stat -c %a "$WALLET.newpassword") == 600 ]]
            fi
            for FILE in admin readonly invoice; do docker exec "$LND" test -s "/data/$FILE.macaroon"; done
            if [[ "$SCENARIO" == custom ]]; then
                # lncli must leave a running daemon's config and authentication files alone.
                docker exec "$LND" sha256sum /data/lnd.conf /data/admin.macaroon /data/data/chain/bitcoin/regtest/macaroons.db > "$WORK/cli-before"
                docker run --rm -v "$VOLUME:/data" -v "$ROOT/docker-entrypoint.sh:/docker-entrypoint.sh:ro" \
                    -e LND_MACAROON_ROTATION_ID=cli "$IMAGE" lncli --version >/dev/null
                docker exec "$LND" sha256sum /data/lnd.conf /data/admin.macaroon /data/data/chain/bitcoin/regtest/macaroons.db > "$WORK/cli-after"
                diff -u "$WORK/cli-before" "$WORK/cli-after"
                docker exec "$LND" test ! -e /data/.macaroon-rotated-cli
            fi
