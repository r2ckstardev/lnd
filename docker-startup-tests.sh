...
for SCENARIO in ${TEST_SCENARIOS:-legacy newline stored-newline empty null omitted password-only custom custom-newline custom-stored-newline custom-long pending-before pending-after rotation rotation-newline rotation-custom rotation-custom-newline rotation-pending rotation-split-store missing-store-old missing-store-new missing-readonly split-store unknown invalid-json fresh interrupted-init}; do
